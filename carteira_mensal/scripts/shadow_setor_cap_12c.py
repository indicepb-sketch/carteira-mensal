...
OUTPUT_FILE = EXCEL_DIR / "shadow_setor_cap_12c.xlsx"
LOG_FILE = LOG_DIR / "shadow_setor_cap_12c.log"
LAMBDA_BETA = 1.5
...
SCENARIOS = [
    ...
    Scenario("cap_dinamico_setor", "medium", "Score medio + maximo 1 acao em setor fraco/deteriorando e teto menor em deterioracao.", True),
]
CURRENT_MES = ""
...
def apply_dynamic_sector_cap(frame: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    out["_setor12c_rank_signal"] = pd.to_numeric(out.get(signal_col, pd.Series(0, index=out.index)), errors="coerce").fillna(0)
    out["setor12c_max_acoes_setor"] = np.where(
    ...
    for sector, group in out.groupby("setor", dropna=False):
        limit = int(group["setor12c_max_acoes_setor"].min()) if not group.empty else 2
        ordered = group.sort_values(["_setor12c_rank_signal", "nota_final"], ascending=[False, False])
    ...
    capped["setor12c_removido_por_cap_setorial"] = False
    if dropped:
...
def recompute_portfolio_metrics(portfolio: pd.DataFrame, covariance: pd.DataFrame, settings: dict, metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    out.update({
        ...
        "setor12c_metricas_recalculadas": True,
    })
    return out
...
def build_with_sector_overlay(scored: pd.DataFrame, covariance: pd.DataFrame, settings: dict) -> tuple[pd.DataFrame, dict, pd.DataFrame]:
    ...
    if not portfolio.empty:
        ...
        if scenario.dynamic_sector_cap:
            portfolio["setor12c_peso_antes_cap"] = pd.to_numeric(portfolio["peso_recomendado"], errors="coerce").fillna(0.0)
            ...
            portfolio["setor12c_teto_individual"] = caps
            portfolio["setor12c_teto_reduzido"] = is_deteriorating
    ...
    metrics["setor12c_cenario"] = scenario.name
    metrics["setor12c_descricao"] = scenario.description
    metrics["setor12c_forca"] = scenario.strength
    metrics["setor12c_dynamic_cap"] = scenario.dynamic_sector_cap
    metrics["setor12c_cap_reasons"] = " | ".join(cap_reasons)
...
def main() -> None:
    ...
    log("Teste 12C - Cap Dinamico por Setor")
