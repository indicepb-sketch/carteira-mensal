...
OUTPUT_FILE = t30.OUTPUT_FILE.parent / "shadow_teste32_recuperacao_restritas_evidenciada.xlsx"
LOG_FILE = t30.LOG_FILE.parent / "shadow_teste32_recuperacao_restritas_evidenciada.log"

SCENARIOS_32 = [
    {"name": "base_28d", "risk_mode": "queda_confirmada_28d", "rule": "base", "cap": np.nan},
    {"name": "restrita_defensiva_queda_confirmada_cap5", "risk_mode": "queda_confirmada_28d", "rule": "defensiva_queda_confirmada", "cap": 0.05},
    {"name": "restrita_virada_forca_cap5", "risk_mode": "queda_confirmada_28d", "rule": "virada_forca", "cap": 0.05},
    {"name": "restrita_combinada_31_cap5", "risk_mode": "queda_confirmada_28d", "rule": "combinada_31", "cap": 0.05},
]
...
def recoverable_by_rule(frame: pd.DataFrame, scenario: dict[str, Any]) -> pd.Series:
    ...
    defensiva_queda = base & confirmed & bucket.str.contains("queda", na=False) & beta.lt(0.9) & (cv.lt(50) | cv.isna())
    virada_forca = base & note.ge(50) & force_ok & trend.str.contains("virada|alta_aceitavel", na=False)
...
def recover_restricted_candidates_32(frame: pd.DataFrame, scenario: dict[str, Any]) -> pd.DataFrame:
    ...
    if not mask.any():
        out["teste32_recuperada_restricao"] = False
        return out
    for col in [
        ...
        "teste32_motivo_recuperacao",
    ]:
        if col not in out.columns:
            out[col] = ""
        out[col] = out[col].fillna("").astype(str)
    out["teste32_recuperada_restricao"] = False
    out.loc[mask, "teste32_recuperada_restricao"] = True
    ...
    out.loc[mask, "teste32_motivo_recuperacao"] = f"recuperacao restrita evidenciada: {scenario['rule']}; cap={cap:.3f}"
    out.loc[mask, "penalizacoes_otimizacao"] = out.loc[mask, "penalizacoes_otimizacao"].map(
        lambda x: t30.append_token(x, f"teste32_{scenario['rule']}_cap_{cap:.3f}")
    )
...
def run_mode_32(
    ...
    try:
        for mes in t30.t28d.t28b.r16.MONTHS:
            ...
            rec_col = "teste32_recuperada_restricao"
            ...
            if not cand.empty:
                cand["cenario_teste32"] = scenario["name"]
                cand["mes"] = mes
...
def summarize(group: pd.DataFrame) -> dict[str, Any]:
    renamed = group.rename(columns={"cenario_teste32": "cenario_teste28b"})
    row = t30.t28d.t28b.summarize(renamed)
    row["cenario_teste32"] = row.pop("cenario_teste28b")
...
def recovered_rows(candidates: pd.DataFrame) -> pd.DataFrame:
    if candidates.empty or "teste32_recuperada_restricao" not in candidates.columns:
        return pd.DataFrame()
    mask = candidates["teste32_recuperada_restricao"].map(t30.t28d.t28b.sh.to_bool)
    cols = [
        "cenario_teste32", "mes", "ticker", "setor", "bucket_regime_previsto", "decisao_preliminar_ajustada",
        ...
        "beta", "cv", "qualidade_fundamentalista", "teste32_motivo_recuperacao",
    ]
...
def main() -> None:
    ...
    summary = pd.DataFrame([summarize(g) for _, g in monthly.groupby("cenario_teste32", sort=False)])
    by_regime = pd.DataFrame([summarize(g) | {"tipo_regime_expost": keys[1]} for keys, g in monthly.groupby(["cenario_teste32", "tipo_regime_expost"], sort=False)])
    baseline = monthly[monthly["cenario_teste32"].eq("base_28d")][["mes", "retorno_modelo", "alfa_modelo", "tickers_pesos"]]
    compare = monthly[~monthly["cenario_teste32"].eq("base_28d")].merge(baseline, on="mes", how="left", suffixes=("", "_base28d"))
    ...
    portfolios = t30.portfolio_rows(results_by_mode).rename(columns={"cenario_teste30": "cenario_teste32"})
    recovered = recovered_rows(candidates)
    validation = t30.validation_rows(monthly.rename(columns={"cenario_teste32": "cenario_teste30"}), portfolios.rename(columns={"cenario_teste32": "cenario_teste30"})).rename(columns={"cenario_teste30": "cenario_teste32"})
    ...
    log("Teste 32 - Recuperacao de Restritas por Evidencia do Teste 31")
    log("Base: 28D. Regras seletivas: defensiva em queda confirmada, virada+forca, e combinada.")
    for _, row in summary.iterrows():
        log(
            f"  {row['cenario_teste32']}: retorno={pct(row['retorno_carteira'])}; IBOV={pct(row['retorno_ibov'])}; "
    ...
    base = summary[summary["cenario_teste32"].eq("base_28d")].iloc[0]
    for scenario in [s["name"] for s in SCENARIOS_32 if s["name"] != "base_28d"]:
        row = summary[summary["cenario_teste32"].eq(scenario)].iloc[0]
        log(
