...
OUTPUT_FILE = ROOT / "output" / "excel" / "shadow_regime_20_exposicao_alta.xlsx"
LOG_FILE = ROOT / "output" / "logs" / "shadow_regime_20_exposicao_alta.log"
...
PROFILES = [
    ...
    {
        "perfil_exposicao": "100_alta_0_quedas",
        "descricao": "100% em acoes apenas em alta; 0% em quedas.",
        "queda_leve": 0.0,
        "queda_forte": 0.0,
    },
    {
        "perfil_exposicao": "100_alta_40_0",
        "descricao": "100% em alta; 40% em queda leve; 0% em queda forte.",
        "queda_leve": 0.40,
        "queda_forte": 0.0,
    },
    {
    ...
    {
        "perfil_exposicao": "100_alta_70_40",
        "descricao": "100% em alta; 70% em queda leve; 40% em queda forte.",
        "queda_leve": 0.70,
        "queda_forte": 0.40,
    },
]
...
def main() -> None:
    ...
    by_realized_regime = summarize_by(monthly, ["cenario", "perfil_exposicao", "tipo_regime_expost"])
    by_predicted_regime = summarize_by(monthly, ["cenario", "perfil_exposicao", "bucket_regime"])

    control = summary[summary["perfil_exposicao"].eq("atual_teste16")][
        ["cenario", "retorno_carteira", "alfa_composto", "drawdown_carteira", "drawdown_relativo_vs_ibov"]
    ].rename(
        columns={
            "retorno_carteira": "retorno_carteira_atual",
            "alfa_composto": "alfa_atual",
            "drawdown_carteira": "drawdown_carteira_atual",
            "drawdown_relativo_vs_ibov": "drawdown_relativo_atual",
        }
    )
    ...
    compare["delta_drawdown_vs_atual"] = compare["drawdown_carteira"] - compare["drawdown_carteira_atual"]
    ...
    with pd.ExcelWriter(OUTPUT_FILE, engine="openpyxl") as writer:
        ...
        by_realized_regime.to_excel(writer, sheet_name="por_regime_realizado", index=False)
        by_predicted_regime.to_excel(writer, sheet_name="por_regime_previsto", index=False)
        monthly.to_excel(writer, sheet_name="mes_a_mes", index=False)

    log("Teste 20 - 100% investido apenas em alta e calibracao de exposicao nas quedas")
    log(f"Meses avaliados: {base['mes'].nunique()} ({base['mes'].min()} a {base['mes'].max()})")
    log("")
    for scenario in SCENARIOS:
        sub = compare[compare["cenario"].eq(scenario)].sort_values("alfa_composto", ascending=False)
        if sub.empty:
            continue
        best = sub.iloc[0]
        pure = sub[sub["perfil_exposicao"].eq("100_alta_0_quedas")].iloc[0]
        atual = sub[sub["perfil_exposicao"].eq("atual_teste16")].iloc[0]
        log(f"{scenario}:")
        log(
            f"  atual Teste16: retorno={pct(atual['retorno_carteira'])}; alfa={pct(atual['alfa_composto'])}; "
            f"drawdown={pct(atual['drawdown_carteira'])}"
        )
        log(
            f"  100% so em alta: retorno={pct(pure['retorno_carteira'])}; alfa={pct(pure['alfa_composto'])}; "
            f"delta_alfa={pct(pure['delta_alfa_vs_atual'])}; drawdown={pct(pure['drawdown_carteira'])}"
        )
        log(
            f"  melhor perfil testado: {best['perfil_exposicao']} | retorno={pct(best['retorno_carteira'])}; "
            f"alfa={pct(best['alfa_composto'])}; delta_alfa={pct(best['delta_alfa_vs_atual'])}; "
            f"drawdown={pct(best['drawdown_carteira'])}; exposicao_media={pct(best['exposicao_media'])}"
        )
    log("")
