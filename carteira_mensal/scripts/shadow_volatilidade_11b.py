...
OUTPUT_FILE = ROOT / "output" / "excel" / "shadow_volatilidade_11b.xlsx"
LOG_FILE = ROOT / "output" / "logs" / "shadow_volatilidade_11b.log"
...
SCENARIOS = [
    Scenario("baseline_consolidado", 1.0, False, "Sem ajuste por volatilidade."),
    Scenario("alerta_apenas", 1.0, False, "Apenas carrega o alerta; pesos iguais ao baseline."),
    Scenario("penalizacao_leve", 0.90, False, "vol_ratio_21 > 1.5 reduz o sinal V3 em 10%."),
    Scenario("penalizacao_forte", 0.75, False, "vol_ratio_21 > 1.5 reduz o sinal V3 em 25%."),
    Scenario("teto_10pct", 1.0, True, "vol_ratio_21 > 1.5 limita peso individual a 10%."),
]
...
def attach_volatility(frame: pd.DataFrame, mes: str) -> pd.DataFrame:
    ...
    out["alerta_volatilidade_anormal_21"] = pd.to_numeric(out["vol_ratio_21"], errors="coerce") > VOL_THRESHOLD
    return out
    ...
    out.update(
        {
            ...
            "volatilidade_11b_recalculou_metricas": True,
        }
    )
    return out
...
def build_with_volatility_overlay(scored: pd.DataFrame, covariance: pd.DataFrame, settings: dict) -> tuple[pd.DataFrame, dict, pd.DataFrame]:
    ...
    if scenario.penalty_factor < 1.0 and not adjusted.empty:
        mask = adjusted["alerta_volatilidade_anormal_21"].fillna(False).astype(bool)
        for col in ("_shadow_objetivo_sinal_norm", "shadow_tamanho_livre_sinal_v3", "score_prioridade_otimizacao"):
            if col in adjusted.columns:
                ...
                adjusted[f"{col}_antes_vol11b"] = numeric_col
                adjusted[col] = numeric_col
                adjusted.loc[mask, col] = numeric_col.loc[mask] * scenario.penalty_factor
        adjusted["vol11b_penalizacao_sinal"] = np.where(mask, 1.0 - scenario.penalty_factor, 0.0)
    else:
        adjusted["vol11b_penalizacao_sinal"] = 0.0
    ...
    metrics["vol11b_cenario"] = scenario.name
    metrics["vol11b_descricao"] = scenario.description
    metrics["vol11b_threshold"] = VOL_THRESHOLD
    metrics["vol11b_penalty_factor"] = scenario.penalty_factor

    if not portfolio.empty:
        portfolio = attach_volatility(portfolio, CURRENT_MES)
        portfolio["vol11b_peso_antes_ajuste"] = portfolio["peso_recomendado"]
        if scenario.use_weight_cap:
            high_vol = portfolio["alerta_volatilidade_anormal_21"].fillna(False).astype(bool)
            ...
            portfolio["vol11b_teto_individual"] = caps
            portfolio["vol11b_cap_10pct_aplicado"] = high_vol
            portfolio["peso_recomendado"] = new_weights.to_numpy(float)
            portfolio["peso_final"] = portfolio["peso_recomendado"]
            metrics["vol11b_cap_10pct_ativos"] = int(high_vol.sum())
            metrics = recompute_metrics(portfolio, covariance, settings, metrics)
        else:
            portfolio["vol11b_teto_individual"] = 0.25
            portfolio["vol11b_cap_10pct_aplicado"] = False
        metrics["vol11b_ativos_alerta_na_carteira"] = int(portfolio["alerta_volatilidade_anormal_21"].fillna(False).astype(bool).sum())
        metrics["vol11b_peso_alerta_na_carteira"] = float(portfolio.loc[portfolio["alerta_volatilidade_anormal_21"].fillna(False).astype(bool), "peso_recomendado"].sum())
...
def candidate_alert_rows(results: dict[tuple[str, str], dict[str, Any]]) -> pd.DataFrame:
    ...
    for (cenario, mes), result in results.items():
        ...
        mask = candidates["alerta_volatilidade_anormal_21"].fillna(False).astype(bool)
        cols = [c for c in ["ticker", "nome", "setor", "nota_final", "forca_relativa_score", "score_prioridade_otimizacao", "_shadow_objetivo_sinal_norm", "shadow_tamanho_livre_sinal_v3", "vol_ratio_21", "vol_bucket_21", "vol11b_penalizacao_sinal", "status_para_risco", "tipo_timing", "decisao_preliminar_ajustada"] if c in candidates.columns]
...
def main() -> None:
    ...
    log("Teste 11B: carteiras sombra com regra de volatilidade anormal.")
    ...
    try:
        for scenario in SCENARIOS:
            ...
            for mes in MONTHS:
                ...
                row["n_ativos_alerta_vol_carteira"] = result.get("metrics", {}).get("vol11b_ativos_alerta_na_carteira", 0)
                row["peso_alerta_vol_carteira"] = result.get("metrics", {}).get("vol11b_peso_alerta_na_carteira", 0.0)
                row["cap_10pct_ativos"] = result.get("metrics", {}).get("vol11b_cap_10pct_ativos", 0)
